...
class ConsultationController extends Controller
{
    ...
    public function UpdateConsultation(Request $request, $id){
        ...
        [
            ...
            'Files.*'   =>  'required|max:25001|mimes:bmp,jpg,jpeg,png,avi,mpg,mpeg,mov,mp4,pdf,zip|file|bail',
            'toDelete'  =>  'array',
            'toDelete.*'    =>  "required|exists:fichiers,id"
        ],
        [
            ...
            'Files.*.mimes' =>  ' types des fichiers supportés: bmp, jpg, jpeg, png, avi, mpg, mpeg, mov, mp4, pdf, zip',
            'Files.*.file' =>  ' Fichier corrupté ',
            'toDelete.array'    =>  'Données invalides!',
            'toDelete.*.required'   => "Données manquantes",
            'toDelete.*.exists'   => " Données invalides ",
        ]
        );
        ...
        $consultation->Description = $request->input('Description');
        $consultation->ExamensAfaire = $request->input('analyses') ? $request->input('analyses') : null ;
        $consultation->save();


        Examen::where('ConsultationId', $consultation->id)->delete();
        Operations_Selon_Consultation::where('ConsultationID', $consultation->id)->delete();
        $ordonnance = Ordonnance::where('ConsultationId', $consultation->id)->first();
        if($ordonnance){
            Medicament_par_ordonnance::where('OrdonnanceId',$ordonnance->id)->delete();
            $ordonnance->delete();
        }
        $facture = Facture::where('ConsultationId', $consultation->id)->first();
        
        if($request->input('toDelete'))
        foreach( $request->input('toDelete') as $todel ){
            $fichier = Fichier::find($todel);
            if( ! $fichier )
                continue;
            $fichier->Delete_Physically();
        }
        ...
        if($medicaments && is_array($medicaments)){
            ...
            $ordonnance = new Ordonnance();
            $ordonnance->ConsultationId = $consultation->id ; 
            $ordonnance->Description = $request->input('AddContent') ? $request->input('AddContent'):null ;
            $ordonnance->save();
        ...
        return response()->json([ 'status' => 'Good','dif'=>$dif ]);
